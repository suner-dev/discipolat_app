...
| G1.3 | ✅ vert | 2026-09-15 | `feat(tenant): complete tenant feature CRUD and enforcement 29` | Migration V143 `tenant_features` (module_code, enabled, configuration_json, limits_json, UNIQUE tenant_id+module_code) · Entité `TenantFeature` + `TenantFeatureRepository` scoped · `TenantFeatureService` (enable/disable/upsert/updateConfiguration/getEnabledFeatures) · `TenantFeatureController` GET/PUT/DELETE `/api/v1/admin/tenant-features` (RBAC TENANT_OWNER/TENANT_ADMIN) · Audit log sur chaque mutation. |
