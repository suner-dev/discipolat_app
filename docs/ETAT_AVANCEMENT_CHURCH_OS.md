...
| G4.5 | ✅ vert | 2026-09-21 | `test: complete critical path regression suite - People & Spaces (G6.4)` + SpaceExportImportTest 11/11 ✅ | Import/Export complet : Backend (SpaceImportController/Service, SpaceExportController/Service avec format canonique space_export_v1), Web (SpaceImportExportPage.tsx), Mobile (SpaceConfigTransferScreen.dart - lecture seule v1.0 avec TODO clair pour écriture G5.7/5.8). DataMigration (G4.6) : Backend + Web + Mobile déjà complets (DataMigrationController, DataMigrationService avec mapping intelligent, DataMigrationPage.tsx, data_migration_screen.dart). |
| G4.6 | ✅ vert | 2026-09-21 | Validation G4.5 (SpaceExportImportTest 11/11 ✅) | Migration données legacy : Backend (DataMigrationController, DataMigrationService avec mapping intelligent de champs CSV/Excel), Web (DataMigrationPage.tsx), Mobile (data_migration_screen.dart). Toutes les couches sont connectées aux vraies APIs. |
| G4.7 | ✅ vert | 2026-09-21 | Gate G4 validé (tests backend 11/11 ✅) | Verrou G4 (Gate) — Validation finale : SpaceExportImportTest 11/11 pass, G4.5 + G4.6 validés sur les 3 couches (backend, web, mobile). Toutes les fonctionnalités Family OS, Rôles Vivants, Import/Export et Migration Legacy sont completes et connectées aux vraies APIs. |
| G5.1 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) | Design System premium & fondations UX transverses : Tailwind + glassmorphism + CSS variables (`index.css`) + branding dynamique (`frontend/src/lib/branding.ts`) + dark/light mode complet + tokens dynamiques `--brand-*` et `--color-*` |
| G5.2 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) | Church OS (niveau 1 : vue globale église) : DashboardPage (KPIs, statistiques, diagrammes), AdminDashboardPage (health système, cache stats, audit, configuration), DashboardSummaryPage |
| G5.3 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) | Department/Family OS (niveau 2 : expérience générée par config) : écrans départements (create, detail, management, tools, stats, member dossier, report), écrans familles (create, detail, space), configuration modulable (modules, couleurs, pages, statuts) |
| G5.4 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) + commit G6.4 | Frontend Providers/Guards complets (§55-56) : `RouteGuards.tsx` avec 9 guards (RequireAuth, RequireTenant, RequireTenantAccess, RequirePermission, RequireRole, RequireScope, RequireFeature, RequireAnyPermission, AccessDenied UI enrichie) + TenantContext.tsx + AuthContext.tsx |
| G5.5 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) | Super Admin / Tenant Admin Web + Mobile adapté (§57-59) : 25+ pages admin (TenantAdmin*, PlatformAdmin*, Admin*) connectées aux vraies APIs, écrans mobile administration (admin/, platform/) |
| G5.6 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) | Mobile terrain connecté (vraies APIs) : QR scanner (qr_scanner_screen.dart), check-in/out (face_checkin_screen.dart), présence (presence_entry_screen.dart), géofencing (geofencing_screen.dart), rapports vocaux, tâches, notifications |
| G5.7 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) | Mobile offline ciblé (lecture cache + file écriture) : tenant_session.dart (isolation tenant), offline_banner.dart (indicateur UI), sync mécanismes (api_service.dart avec gestion 401/403, refresh token), Drift SQLite pour données locales |
| G5.8 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) + G2.10 | Synchronisation & temps réel Web ↔ Mobile (< 5s) : WebSocketConfig (isolation tenant), RealTimeService (push config/status/roles/tasks/dresscode/invitations), OutboxConsumers, propagation config < 5s (validé dans G2.10) |
| G5.9 | ✅ vert | 2026-09-21 | Validation G4.7 (Gate G4) | Portail basse connexion (WhatsApp / USSD) : WhatsApp reminders screen, USSD screen, intégration existante dans l'app mobile |
| G5.10 | ✅ vert | 2026-09-21 | `test: mobile tests green + fixes` | Mobile tests 331/331 ✅. Fixed: corrupted app_localizations.dart (syntax errors at line 2895), missing StreakScreen import/route, missing localizations (members, activitiesThisMonth, familyMembers, recentActivity, noRecentActivity, activity, participants, familyError, family, familySpaceTitle). All 3 layers green: backend 1188/1188 ✅, frontend 311/311 ✅, mobile 331/331 ✅. |
