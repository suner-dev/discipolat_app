...
| G6.4 | 🔵 en cours | 2026-09-18 | `test: critical path regression suite (70)` | Backend integration tests for critical paths: People (auto-registration → directory → assignment), Spaces (template creation → customization → propagation), Events (dress code + archives), Assets (checkout/damage → maintenance → finance), Workflows (approval cycle), Roles (interface reconfiguration < 5s), Invitations (email → acceptance → membership), Finance (reconciliation). Spring WebTestClient + MockMvc. Frontend Vitest + Playwright E2E to be added. |
