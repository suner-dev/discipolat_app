...
|---|---|---|
| Page Builder (blocs : KPI, tableau, graphique, formulaire, calendrier, timeline, checklist…) | A (implémenté session 08-17) | **Fait (V65+V66)** : CRUD de pages personnalisées versionnées (config_revisions), blocs KPI/TABLEAU/LISTE/TEXTE/LIENS/RECHERCHE/IMAGES **+ GRAPHIQUE/CALENDRIER/TIMELINE/CHECKLIST (V66)** résolus côté serveur sur données réelles scopées (aucune statistique fictive), publication + version, RBAC par rôles au rendu, éditeur web (`/admin/pages`) + supervision mobile (liste/publication/suppression). Graphiques : camembert/barres/courbe (4 sources : âmes par statut, événements par mois, alertes par type, départements par statut) ; calendrier (60 j, navigation mensuelle) ; timeline (10 dernières âmes) ; checklist (persistance locale, progression). Reste : blocs formulaire/fichiers/tâches |
