...
void main() {
  group('AiVisitNotesScreen', () {
    testWidgets('renders app bar', (tester) async {
      await pumpLocalized(tester, AiVisitNotesScreen(apiService: _FakeApiService()));
    ...
    testWidgets('shows empty state', (tester) async {
      await pumpLocalized(tester, AiVisitNotesScreen(apiService: _FakeApiService(empty: true)));
      await tester.pumpAndSettle();
      expect(find.text('Aucune note de visite analysée.'), findsOneWidget);
    });

    testWidgets('shows error state with retry', (tester) async {
      await pumpLocalized(tester, AiVisitNotesScreen(apiService: _FakeApiService(fail: true)));
      await tester.pumpAndSettle();
