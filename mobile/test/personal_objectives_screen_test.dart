...
void main() {
  group('PersonalObjectivesScreen', () {
    testWidgets('renders app bar', (tester) async {
      await pumpLocalized(tester, PersonalObjectivesScreen(apiService: _FakeApiService()));
    ...
    testWidgets('shows empty state', (tester) async {
      await pumpLocalized(tester, PersonalObjectivesScreen(apiService: _FakeApiService(empty: true)));
      await tester.pumpAndSettle();
      expect(find.text('Aucun objectif défini.'), findsOneWidget);
    });

    testWidgets('shows error state with retry', (tester) async {
      await pumpLocalized(tester, PersonalObjectivesScreen(apiService: _FakeApiService(fail: true)));
      await tester.pumpAndSettle();
