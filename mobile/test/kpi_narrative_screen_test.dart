...
void main() {
  group('KpiNarrativeScreen', () {
    testWidgets('renders app bar', (tester) async {
      await pumpLocalized(tester, KpiNarrativeScreen(apiService: _FakeApiService()));
    ...
    testWidgets('shows empty state', (tester) async {
      await pumpLocalized(tester, KpiNarrativeScreen(apiService: _FakeApiService(empty: true)));
      await tester.pumpAndSettle();
      expect(find.text('Aucune narration générée. Utilisez « Générer » côté web.'), findsOneWidget);
    });

    testWidgets('shows error state with retry', (tester) async {
      await pumpLocalized(tester, KpiNarrativeScreen(apiService: _FakeApiService(fail: true)));
      await tester.pumpAndSettle();
