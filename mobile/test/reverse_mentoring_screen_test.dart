...
void main() {
  group('ReverseMentoringScreen', () {
    testWidgets('renders app bar', (tester) async {
      await pumpLocalized(tester, ReverseMentoringScreen(apiService: _FakeApiService()));
    ...
    testWidgets('shows empty state', (tester) async {
      await pumpLocalized(tester, ReverseMentoringScreen(apiService: _FakeApiService(empty: true)));
      await tester.pumpAndSettle();
      expect(find.text('Aucune demande de mentorat inversé.'), findsOneWidget);
    });

    testWidgets('shows error state with retry', (tester) async {
      await pumpLocalized(tester, ReverseMentoringScreen(apiService: _FakeApiService(fail: true)));
      await tester.pumpAndSettle();
