...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _logout() async {
    final tenantSession =
        ProviderScope.containerOf(context).read(tenantSessionProvider);
    if (ImpersonationService.instance.isImpersonating) {
      await ImpersonationService.instance.stop();
    }
    ...
    await ImpersonationService.instance.clearBackup();
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    children: [
                      ...
                      StatusBadge(
                          label: role, color: Colors.green, glowing: true),
                      ...
                      Row(
                        children: [
                          _statCard(
                              Icons.star,
                              AppLocalizations.of(context).profileScore,
                              '${_dashboard?['scoreSpirituel'] ?? '—'}',
                              Colors.amber),
                          const SizedBox(width: 8),
                          _statCard(
                              Icons.check_circle,
                              AppLocalizations.of(context).profilePresence,
                              '${_dashboard?['tauxPresence'] ?? '—'}',
                              Colors.green),
                          const SizedBox(width: 8),
                          _statCard(
                              Icons.trending_up,
                              AppLocalizations.of(context).profileProgression,
                              '${_dashboard?['progression'] ?? '—'}',
                              Colors.blue),
                        ],
                      ),
                      const SizedBox(height: 16),
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _profileRow(Icons.person_outline,
                                AppLocalizations.of(context).profileRole, role),
                            const GlassDivider(),
                            _profileRow(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _profileRow(
                                Icons.auto_awesome,
                                AppLocalizations.of(context)
                                    .profileSpiritualScore,
                                '${_dashboard?['scoreSpirituel'] ?? '—'}/100'),
                            const GlassDivider(),
                            _profileRow(
                                Icons.family_restroom,
                                AppLocalizations.of(context).profileFamily,
                                _dashboard?['famille'] ?? '—'),
                            const GlassDivider(),
                            _profileRow(
                                Icons.business,
                                AppLocalizations.of(context).profileDepartment,
                                _dashboard?['departement'] ?? '—'),
                          ],
                        ),
                      ),
                      const SizedBox(height: 16),
