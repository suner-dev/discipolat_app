...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('DISCIPLINE_TYPE', 'TEMOIGNAGE', 'Temoignage', '#22c55e', 7, TRUE),
    ('DISCIPLINE_TYPE', 'ENTRETIEN', 'Entretien pastoral', '#06b6d4', 8, TRUE),
    ('DISCIPLINE_TYPE', 'RESOLUTION', 'Resolution', '#22c55e', 9, TRUE),
    ('DISCIPLINE_TYPE', 'AUTRE', 'Autre', '#6b7280', 10, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Gravite disciplinaire (miroir de l'enum backend GraviteDiscipline)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
...
-- Priorites de feedback (miroir de l'enum backend Feedback.Priority)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ('INTERACTION_CANAL', 'TELEPHONE', 'Telephone', '#3b82f6', 1, TRUE),
    ...
    ('INTERACTION_CANAL', 'VIDEO', 'Visioconference', '#f59e0b', 5, TRUE),
    ('INTERACTION_CANAL', 'PRESENTIEL', 'En presentiel', '#22c55e', 6, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;
