...
-- Complete V42/V44 : statuts de validation des rapports de
-- famille, types/cibles/priorites/statuts d'alertes, types et
...
-- Types d'alertes (miroir des valeurs emises par le backend)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ('ALERT_TYPE', 'ABSENCE_48H', 'Absence 48h', '#ef4444', 1, TRUE),
    ('ALERT_TYPE', 'ABSENCE_3_SEMAINES', 'Decrochage 3 semaines', '#ef4444', 2, TRUE),
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('ALERT_CIBLE', 'DEPARTEMENT', 'Departement', '#a855f7', 2, TRUE),
    ('ALERT_CIBLE', 'FAMILLE', 'Famille', '#22c55e', 3, TRUE),
    ('ALERT_CIBLE', 'GROUPE', 'Groupe', '#f59e0b', 4, TRUE),
    ('ALERT_CIBLE', 'EGLISE', 'Eglise', '#06b6d4', 5, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Priorites des alertes (miroir de l'enum PrioriteAlerte)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('ALERT_STATUS', 'TRAITEE', 'Traitee', '#f59e0b', 2, TRUE),
    ('ALERT_STATUS', 'RESOLUE', 'Resolue', '#22c55e', 3, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('NOTIFICATION_TYPE', 'PRIERE_EXAUCEE', 'Prieres exaucees', '#22c55e', 6, TRUE),
    ...
    ('NOTIFICATION_TYPE', 'TRANSFERT_VALIDEE', 'Transfert valide', '#22c55e', 9, TRUE),
    ('NOTIFICATION_TYPE', 'TRANSFERT_REFUSEE', 'Transfert refuse', '#ef4444', 10, TRUE),
    ('NOTIFICATION_TYPE', 'TRANSFERT_INFOS_DEMANDEES', 'Informations demandees', '#f59e0b', 11, TRUE),
    ('NOTIFICATION_TYPE', 'TRANSFERT_CORRECTION', 'Correction demandee', '#f59e0b', 12, TRUE),
    ('NOTIFICATION_TYPE', 'TRANSFERT_EXECUTEE', 'Transfert execute', '#22c55e', 13, TRUE),
    ('NOTIFICATION_TYPE', 'TRANSFERT_ANNULEE', 'Transfert annule', '#6b7280', 14, TRUE),
    ('NOTIFICATION_TYPE', 'TRANSFERT_DELAI_DEPASSE', 'Delai de traitement depasse', '#ef4444', 15, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;
