...
-- ============================================================
-- DICTIONNAIRES ETENDUS (referentiels configurables)
-- Complete V42 : roles utilisateurs, feedback, transferts,
-- demandes membres, evaluations, discipline, visibilite des
-- prieres, entites d'audit, etat spirituel.
-- L'administrateur renomme, reordonne, colore, ajoute ou
-- desactive chaque valeur — sans modifier le code.
-- ============================================================

-- Roles utilisateurs (libelles affiches dans toute l'application)
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ('USER_CHARGE', 'LEGER', 'Leger', '#22c55e', 1, TRUE),
    ('USER_CHARGE', 'NORMAL', 'Normal', '#3b82f6', 2, TRUE),
    ('USER_CHARGE', 'SURCHARGE', 'Surcharge', '#ef4444', 3, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Categories de feedback testeurs
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('FEEDBACK_CATEGORIE', 'FONCTIONNALITE_MANQUANTE', 'Fonctionnalite manquante', '#a855f7', 4, TRUE),
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('FEEDBACK_STATUS', 'RESOLU', 'Resolu', '#22c55e', 3, TRUE),
    ('FEEDBACK_STATUS', 'REJETE', 'Rejete', '#6b7280', 4, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Types de transfert (miroir de l'enum backend TransferType)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ('TRANSFER_TYPE', 'MEMBRE_DEPARTEMENT_TRANSFERT', 'Transfert de membre entre departements', '#3b82f6', 1, TRUE),
    ('TRANSFER_TYPE', 'MEMBRE_DEPARTEMENT_AJOUT', 'Ajout de membre dans un departement', '#22c55e', 2, TRUE),
    ('TRANSFER_TYPE', 'MEMBRE_DEPARTEMENT_RETRAIT', 'Retrait de membre d''un departement', '#ef4444', 3, TRUE),
    ...
    ('TRANSFER_TYPE', 'RESPONSABLE_DEPARTEMENT_CHANGEMENT', 'Changement du responsable d''un departement', '#ec4899', 8, TRUE),
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('TRANSFER_STATUS', 'VALIDE', 'Valide', '#8b5cf6', 5, TRUE),
    ('TRANSFER_STATUS', 'REFUSE', 'Refuse', '#ef4444', 6, TRUE),
    ('TRANSFER_STATUS', 'ANNULE', 'Annule', '#6b7280', 7, TRUE),
    ('TRANSFER_STATUS', 'EXECUTE', 'Execute', '#22c55e', 8, TRUE),
    ('TRANSFER_STATUS', 'ARCHIVE', 'Archive', '#94a3b8', 9, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Decisions de validation (miroir de l'enum backend DecisionType)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('MEMBER_REQUEST_STATUS', 'RESOLU', 'Resolu', '#22c55e', 3, TRUE),
    ('MEMBER_REQUEST_STATUS', 'REJETE', 'Rejete', '#ef4444', 4, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Visibilites des prieres (miroir de l'enum backend Prayer.Visibilite)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ('PRAYER_VISIBILITE', 'GENERALE', 'General', '#06b6d4', 1, TRUE),
    ...
    ('PRAYER_VISIBILITE', 'PRIVEE', 'Prive', '#6b7280', 5, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Categories d'evaluations (roles evalues)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
...
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('SPIRITUAL_LEVEL', 'EN_DIFFICULTE', 'En difficulte', '#ef4444', 4, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Categories de discipline (registre de discipline)
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('DISCIPLINE_CATEGORIE', 'PONCTUALITE', 'Ponctualite', '#06b6d4', 5, TRUE),
    ('DISCIPLINE_CATEGORIE', 'PARTICIPATION', 'Participation', '#3b82f6', 6, TRUE),
    ('DISCIPLINE_CATEGORIE', 'FIDELITE', 'Fidelite', '#22c55e', 7, TRUE),
    ...
    ('DISCIPLINE_CATEGORIE', 'RELATION_PROBLEMATIQUE', 'Relation problematique', '#ec4899', 14, TRUE),
    ('DISCIPLINE_CATEGORIE', 'FLIRT_INAPPROPRIE', 'Flirt inapproprie', '#ec4899', 15, TRUE),
    ('DISCIPLINE_CATEGORIE', 'DEGAT_MATERIEL', 'Degât materiel', '#6b7280', 16, TRUE),
    ('DISCIPLINE_CATEGORIE', 'DEGAT_RELATIONNEL', 'Degât relationnel', '#6b7280', 17, TRUE)
ON CONFLICT (dict_key, code) DO NOTHING;

-- Entites du journal d'audit
INSERT INTO dictionary_entries (dict_key, code, label, color, ordre, is_default) VALUES
    ...
    ('AUDIT_ENTITY', 'DEPARTMENT', 'Departement', '#a855f7', 5, TRUE),
    ('AUDIT_ENTITY', 'TRANSFER', 'Transfert', '#8b5cf6', 6, TRUE),
    ('AUDIT_ENTITY', 'EVENT', 'Evenement', '#ec4899', 7, TRUE),
