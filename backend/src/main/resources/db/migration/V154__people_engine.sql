...
-- ============================================================
-- G3.1 — People Engine : identité unique + inscription automatique
-- G3.2 — Membership, SpaceMembership & RoleAssignment (3 dim + historisation)
-- Contrat : Annexe A §A.2-A.3
-- ============================================================

-- 1. PERSON (identité unique par tenant, dédoublonnage email/phone)
...
COMMENT ON TABLE person IS 'G3.1 : identité unique par église — une seule fiche par personne';
COMMENT ON COLUMN person.email_normalized IS 'Email normalisé (lowercase, trim) pour dédoublonnage';
COMMENT ON COLUMN person.phone_normalized IS 'Téléphone normalisé (E.164) pour dédoublonnage';
COMMENT ON COLUMN person.visibility_scope IS 'Portée de visibilité dans l\'annuaire (G4.2)';

-- 2. MEMBERSHIP (appartenance à l'église/tenant)
...
COMMENT ON TABLE membership IS 'G3.2 : appartenance à l\'église (tenant) — distincte des espaces';

-- 3. SPACE_MEMBERSHIP (appartenance à un espace : département, famille, sous-équipe)
...
COMMENT ON TABLE space_membership IS 'G3.2 : appartenance à un espace (département/famille/sous-équipe) — dimension 2';
COMMENT ON COLUMN space_membership.membership_type IS 'Type de responsabilité dans l\'espace';
...
COMMENT ON TABLE role_assignment IS 'G3.2 : fonction permanente (rôle org) — dimension 1, historisée (jamais d\'écrasement)';
COMMENT ON COLUMN role_assignment.ended_at IS 'Date de fin du mandat — clôture sans suppression pour historique';

-- 5. EVENT_ASSIGNMENT (fonction événementielle)
CREATE TABLE IF NOT EXISTS event_assignment (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    person_id UUID NOT NULL REFERENCES person(id) ON DELETE CASCADE,
    event_id UUID NOT NULL, -- FK vers events (créé en G3.3)
...
COMMENT ON TABLE event_assignment IS 'G3.2 : affectation événementielle — dimension 3, indépendante des rôles permanents';
...
-- 6. Seed roles catalogue (complément rôles existants)
INSERT INTO roles (id, tenant_id, key, label, description, system, priority) VALUES
    (gen_random_uuid(), NULL, 'HEALTH_STAFF', 'Personnel Santé', 'Infirmier/Médecin de l\'espace santé', TRUE, 250),
    (gen_random_uuid(), NULL, 'HEALTH_LEAD', 'Responsable Santé', 'Responsable de l\'infirmerie', TRUE, 300),
    (gen_random_uuid(), NULL, 'FAMILY_LEADER', 'Chef de Famille', 'Responsable d\'un espace FAMILY', TRUE, 300),
    (gen_random_uuid(), NULL, 'DISCIPLE_MAKER', 'Faiseur de Disciples', 'Accompagnateur spirituel (activé par template FAMILY/DISCIPLESHIP)', TRUE, 200)
