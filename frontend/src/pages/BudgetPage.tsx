...
export default function BudgetPage() {
  ...
  const createMutation = useMutation({ mutationFn: async () => api.post('/budgets', form), onSuccess: () => { toast.success(tText('Budget créé')); setShowForm(false); qc.invalidateQueries({ queryKey: ['budgets'] }); }, onError: (e) => toast.error(getErrorMessage(e)) });
  const deleteMutation = useMutation({ mutationFn: async (id: string) => api.delete(`/budgets/${id}`), onSuccess: () => { toast.success(tText('Supprimé')); qc.invalidateQueries({ queryKey: ['budgets'] }); }, onError: (e) => toast.error(getErrorMessage(e)) });
  ...
  return (
    <div className="page-container">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="stat-card"><TrendingUp className="w-5 h-5 text-emerald-500" /><p className="stat-value">{totalAllocated.toLocaleString(locale)} F</p><p className="stat-label">{tText('Total alloué')}</p></div>
        <div className="stat-card"><TrendingDown className="w-5 h-5 text-red-500" /><p className="stat-value">{totalSpent.toLocaleString(locale)} F</p><p className="stat-label">{tText('Total dépensé')}</p></div>
      ...
      {showForm && (
        <div className="glass-card p-6 mb-6 space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div><label className="text-xs font-medium text-gray-500 mb-1 block">{tText('Catégorie')}</label><input className="input w-full" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} /></div>
            ...
            <div><label className="text-xs font-medium text-gray-500 mb-1 block">{tText('Période')}</label><select className="input w-full" value={form.period} onChange={(e) => setForm({ ...form, period: e.target.value })}><option value="monthly">Mensuel</option><option value="quarterly">Trimestriel</option><option value="yearly">Annuel</option></select></div>
          </div>
          <div className="flex justify-end gap-3"><button onClick={() => setShowForm(false)} className="btn-sm px-4 py-2 rounded-lg glass-card">{tText('Annuler')}</button><button onClick={() => createMutation.mutate()} disabled={!form.name.trim()} className="btn-primary btn-sm">{tText('Créer')}</button></div>
        </div>
      )}
