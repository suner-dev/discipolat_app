...
import { Plus, Trash2, Loader2, Search, Wrench } from 'lucide-react';
...
export default function MemberCompetencesPage() {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(tText('Compétence ajoutée'));
      setShowAdd(false);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success(tText('Supprimé')); queryClient.invalidateQueries({ queryKey: ['member-competences'] }); },
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <div>
          <h1 className="page-title">{tText('Compétences des Membres')}</h1>
          <p className="page-subtitle">{tText('Gérez les compétences et expertise de vos membres')}</p>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="glass-card p-4 text-center">
            ...
            <div className="text-xs text-gray-500">{tText('Total compétences')}</div>
          </div>
          <div className="glass-card p-4 text-center">
            <div className="text-2xl font-bold text-green-600">{stats.certifiedCount}</div>
            <div className="text-xs text-gray-500">{tText('Certifiées')}</div>
          </div>
              ...
              <div key={c.id} className="bg-white dark:bg-white/5 rounded-xl p-4 border border-gray-200 dark:border-white/10 flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {c.certified && <span className="px-2 py-0.5 rounded-full bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400 text-xs">{tText('Certifié')}</span>}
                  </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-gray-200 dark:border-white/10">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">{tText('Ajouter une compétence')}</h2>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowAdd(false)} className="px-4 py-2 rounded-xl border text-sm">{tText('Annuler')}</button>
