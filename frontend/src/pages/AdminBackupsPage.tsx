            ...
            {(listQ.data ?? []).map((b) => (
              <div key={b.id} className="flex flex-wrap items-center justify-between gap-3 bg-black/20 rounded-xl px-4 py-3 text-sm">
                <div>
                  ...
                  <span className="text-gray-500 ml-3">{fmtSize(b.tailleOctets)}{b.executedAt ? ` • exécutée le ${new Date(b.executedAt).toLocaleString(getI18nLocale())}` : ''}</span>
                </div>
                <div className="flex items-center gap-3">
                  {b.integriteOk === true && <span title={tText('Intégrité vérifiée')}><ShieldCheck className="w-4 h-4 text-green-400" /></span>}
                  ...
                  <button onClick={() => trigger.mutate(b.id)} disabled={trigger.isPending} aria-label={tText('Déclencher la sauvegarde')} className="text-sky-400 hover:text-sky-300"><Play className="w-4 h-4" /></button>
