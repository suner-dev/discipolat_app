...
export default function UpcomingEventsMemberPage() {
  ...
  if (isLoading) return <div className="p-6 text-gray-400">{tText('Chargement de votre calendrier…')}</div>;

  return (
    <div className="space-y-6 p-6">
      <h1 className="text-2xl font-bold text-white flex items-center gap-2"><Calendar className="text-cyan-400" /> {tText('Mes événements à venir')}</h1>
          ...
          return (
            <div key={id} className="bg-white/5 backdrop-blur rounded-xl p-4 border border-white/10 flex flex-wrap items-center gap-4 hover:border-cyan-500/30 transition">
              <div className="w-14 h-14 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 rounded-xl flex flex-col items-center justify-center border border-cyan-500/20 shrink-0">
                <span className="text-xs text-cyan-400 font-bold">{d ? d.toLocaleDateString(getI18nLocale(), { weekday: 'short' }) : '—'}</span>
              ...
              <div className="flex-1 min-w-[140px]">
                ...
                <p className="text-xs text-gray-400">{e.type ? `${e.type} • ` : ''}{dateStr}{d ? ` • ${d.toLocaleTimeString(getI18nLocale(), { hour: '2-digit', minute: '2-digit' })}` : ''}</p>
              </div>
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => setRsvp.mutate({ eventId: id, rsvp: 'INTERESTED' })} disabled={setRsvp.isPending}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium ${e.rsvp === 'INTERESTED' ? 'bg-blue-500/30 text-blue-300' : 'text-gray-400 hover:bg-blue-500/10 hover:text-blue-300'}`}><Star className="w-3 h-3 inline" /> {tText('Intéressé')}</button>
                {e.rsvp && e.rsvp !== 'CANCEL' && (
                  <button onClick={() => setRsvp.mutate({ eventId: id, rsvp: 'CANCEL' })} disabled={setRsvp.isPending}
                    className="px-2 py-1.5 rounded-lg text-xs text-gray-500 hover:text-red-400" title={tText('Annuler ma participation')}><X className="w-3 h-3" /></button>
                )}
              </div>
