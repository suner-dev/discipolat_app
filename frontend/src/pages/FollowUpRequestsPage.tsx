  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="bg-white/5 backdrop-blur rounded-2xl p-5 border border-white/10">
        <h2 className="text-white font-semibold mb-3">{tText('Nouvelle demande')}</h2>
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <input value={message} onChange={(e) => setMessage(e.target.value)} placeholder={tText('Décrivez votre besoin en quelques mots…')}
        ...
        {list.map((r) => (
          <div key={r.id} className="bg-white/5 backdrop-blur rounded-xl p-4 border border-white/10 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[11px] text-gray-600 mt-1">{new Date(r.createdAt).toLocaleDateString(getI18nLocale())}{r.requesterName ? ` • ${r.requesterName}` : ''}{r.assignedToName ? ` • assignée à ${r.assignedToName}` : ''}</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              {tab === 'assigned' && r.status !== 'TERMINEE' && (
                <button onClick={() => complete.mutate(r.id)} disabled={complete.isPending} className="flex items-center gap-1 text-green-400 hover:text-green-300 text-xs"><CheckCircle2 className="w-4 h-4" /> {tText('Marquer terminée')}</button>
              )}
            </div>
          </div>
        ))}
