...
export default function EvaluationsPage() {
  ...
  const [allCategorieFilter, setAllCategorieFilter] = useState('');
  ...
  // Global stats for Pasteur view
  const globalStats = useMemo(() => {
    if (!allEvals) return null;
    let totalEvals = 0;
    let totalAvg = 0;
    let avgCount = 0;
    let topPerformer = { nom: '', moyenne: 0 };
    const allPersons: { id: string; nom: string; moyenne: number; total: number }[] = [];

    Object.values(allEvals).forEach((catData) => {
      totalEvals += catData.totalEvaluations;
      if (catData.moyenneGlobale > 0) { totalAvg += catData.moyenneGlobale; avgCount++; }
      catData.parPersonne.forEach((p) => {
        allPersons.push(p);
        if (p.moyenne > topPerformer.moyenne) topPerformer = { nom: p.nom, moyenne: p.moyenne };
      });
    });

    return {
      totalEvals,
      avgGlobal: avgCount > 0 ? (totalAvg / avgCount).toFixed(1) : '—',
      categoriesCount: Object.keys(allEvals).length,
      topPerformer,
  ...
  const filteredAllEvals = useMemo(() => {
    if (!allEvals) return {};
    const result: typeof allEvals = {};
    Object.entries(allEvals).forEach(([cat, data]) => {
      if (allCategorieFilter && cat !== allCategorieFilter) return;
      if (allSearchTerm) {
        const q = allSearchTerm.toLowerCase();
        const filtered = data.parPersonne.filter(p => p.nom.toLowerCase().includes(q));
        if (filtered.length > 0) result[cat] = { ...data, parPersonne: filtered };
      } else {
        result[cat] = data;
      }
    });
    return result;
  }, [allEvals, allSearchTerm, allCategorieFilter]);
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {stats.commentaires.map((c, i) => (
                  <div key={i} className="p-2 rounded-lg bg-gray-50 dark:bg-gray-800/50">
                    <div className="flex items-center gap-1 mb-1">{renderStars(c.note)}</div>
  ...
  return (
    <div className="page-container">
      ...
      {/* Global stats (Pasteur view) */}
      {isPasteurOrAdmin && globalStats && activeTab === 'all' && !selectedUserForPasteur && (
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
          {[
            { label: 'Total évaluations', value: globalStats.totalEvals, icon: BarChart3, color: 'from-primary-500 to-primary-600' },
            { label: 'Moyenne globale', value: globalStats.avgGlobal, icon: Star, color: 'from-amber-500 to-yellow-500' },
            { label: 'Catégories', value: globalStats.categoriesCount, icon: Shield, color: 'from-blue-500 to-indigo-500' },
            { label: 'Personnes évaluées', value: globalStats.totalPersons, icon: Users, color: 'from-emerald-500 to-green-500' },
            { label: 'Top performer', value: globalStats.topPerformer.nom || '—', icon: Trophy, color: 'from-purple-500 to-violet-500' },
          ].map((s, i) => (
            <div key={s.label} className="stat-card animate-slide-up" style={{ animationDelay: `${i * 50}ms` }}>
              ...
              <p className={`stat-value ${typeof s.value === 'string' && s.value.length > 8 ? 'text-base' : 'text-xl'}`}>{s.value}</p>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="flex gap-1">
          ...
          {isPasteurOrAdmin && (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${
                activeTab === 'all' ? 'bg-white dark:bg-gray-800 shadow-sm text-gray-900 dark:text-gray-100' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'
              }`}
            >
          ...
          {selectedPerson ? (
            <div className="card p-6 max-w-lg mx-auto">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea className="input mt-1" rows={3} value={commentaire}
                    onChange={(e) => setCommentaire(e.target.value)} placeholder="Partagez votre appréciation..." />
                </div>
                <button
                  onClick={() => submitMutation.mutate({
                    evalueId: selectedPerson.id, categorie: selectedPerson.categorie,
            ...
            <>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
                ...
                Personnes à évaluer ({peopleToEvaluate?.length || 0})
              </h2>
              {peopleLoading ? (
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {peopleToEvaluate.map((person) => (
                    <button key={person.id + person.categorie} onClick={() => setSelectedPerson(person)}
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card p-4 flex flex-wrap items-center gap-3">
                <div className="relative flex-1 min-w-[200px]">
                  ...
                  <input type="text" placeholder="Rechercher une personne..." value={allSearchTerm}
                    onChange={(e) => setAllSearchTerm(e.target.value)} className="input pl-9" />
                </div>
              ...
              {Object.entries(filteredAllEvals).map(([cat, data]) => (
                <div key={cat} className="glass-card p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${CATEGORIE_COLORS[cat]}`}>{categorieLabel(cat)}</span>
                  ...
                  <div className="space-y-2">
                    {data.parPersonne.map((person, idx) => {
                      ...
                      return (
                        <button key={person.id} onClick={() => setSelectedUserForPasteur(person.id)}
                          ...
                          <span className="w-8 text-sm font-bold text-gray-300 text-center">{medal || `${idx + 1}`}</span>
                          <div className="flex-1">
                            <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{person.nom}</p>
                            <div className="flex items-center gap-2 mt-1">
                              <div className="flex-1 h-1.5 rounded-full bg-gray-100 dark:bg-gray-700 overflow-hidden">
                                <div className="h-full rounded-full bg-gradient-to-r from-amber-400 to-amber-500" style={{ width: `${(person.moyenne / 5) * 100}%` }} />
                              </div>
