  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">{tText('Votre progression spirituelle et journal')}</p>
      </div>
        ...
        <div className="mb-6">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <div className="flex rounded-lg overflow-hidden border border-gray-200 dark:border-white/10">
              <button onClick={() => setJournalView('all')} className={`px-3 py-1.5 text-xs font-medium ${journalView === 'all' ? 'bg-violet-500 text-white' : 'bg-white dark:bg-white/5 text-gray-600 dark:text-gray-300'}`}>{tText('Tous')}</button>
            ...
            <button onClick={() => setShowCreate(true)} className="ml-auto px-3 py-1.5 rounded-lg bg-violet-500 text-white text-xs font-medium hover:bg-violet-600 transition-all flex items-center gap-1">
              <Plus className="w-3 h-3" /> {tText('Nouvelle entrée')}
            </button>
          </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-gray-200 dark:border-white/10">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">{tText('Nouvelle entrée de journal')}</h2>
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-gray-200 dark:border-white/10">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => deleteJournalEntry(selectedEntry.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-red-400" title={tText('Supprimer')}>
      ...
      {loading ? <SkeletonLoader lines={6} variant="card" /> : stages.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">{tText('Votre parcours spirituel sera bientôt disponible')}</p>
        </div>
      ) : (
