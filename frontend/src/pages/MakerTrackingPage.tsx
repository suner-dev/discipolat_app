...
import { Footprints, Plus, Trash2, Loader2, User, Calendar } from 'lucide-react';
...
export default function MakerTrackingPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(tText('Événement enregistré'));
      setShowCreate(false);
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success(tText('Supprimé')); queryClient.invalidateQueries({ queryKey: ['maker-tracking'] }); },
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <div>
          <h1 className="page-title">{tText('Suivi des Faiseurs')}</h1>
      ...
      {resume && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="glass-card p-4 text-center">
            ...
            <div className="text-xs text-gray-500">{tText('Total événements')}</div>
          </div>
            ...
            {events.map(ev => (
              <div key={ev.id} className="bg-white dark:bg-white/5 rounded-xl p-4 border border-gray-200 dark:border-white/10 flex items-center gap-4">
                ...
                <div className="text-xs text-gray-400 flex flex-col items-end gap-1 flex-shrink-0">
                  <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(ev.date || ev.createdAt).toLocaleDateString(getI18nLocale())}</span>
