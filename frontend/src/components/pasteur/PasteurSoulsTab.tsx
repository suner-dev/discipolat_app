...
export default function PasteurSoulsTab() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['souls'] }); toast.success(tText('Âme supprimée')); setShowDeleteConfirm(null); },
    onError: () => toast.error(tText('Erreur lors de la suppression')),
  });

  const restoreMutation = useMutation({
    mutationFn: async (id: string) => { await api.patch(`/souls/${id}/restore`); },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['souls'] }); toast.success(tText('Âme restaurée ✨')); },
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['souls'] }); toast.success(tText('Âme créée avec succès')); setView('liste'); setForm(emptyForm); },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['souls'] }); toast.success(tText('Âme mise à jour')); setView('liste'); setEditingId(null); setForm(emptyForm); },
        ...
        <div className="glass-card p-6">
          <div className="flex items-start justify-between mb-6">
            ...
            <div className="flex gap-2">
              ...
              <Link to={`/souls/${selectedSoul.id}`} className="btn-secondary btn-sm"><Eye className="w-4 h-4" /> {tText('Vue complète')}</Link>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-center">
              <p className="text-xs text-gray-400">{tText('Intégration')}</p>
              <p className="font-semibold text-sm">{new Date(selectedSoul.dateIntegration).toLocaleDateString(getI18nLocale())}</p>
            </div>
            <div className="p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50 text-center">
              <p className="text-xs text-gray-400">Dernier contact</p>
              <p className="font-semibold text-sm">{selectedSoul.dateDernierContact ? new Date(selectedSoul.dateDernierContact).toLocaleDateString(getI18nLocale()) : '—'}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6">
            <Link to={`/souls/${selectedSoul.id}`} className="glass-card p-3 hover:shadow-lg transition-all group">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-gray-900 dark:text-gray-100">{tText('Fiche complète')}</span>
        ...
        <div className="glass-card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input className="input" value={form.nom} onChange={e => setForm({ ...form, nom: e.target.value })} placeholder={tText('Nom de famille')} />
            </div>
            <div>
              <label className="label">{tText('Prénom')}</label>
              <input className="input" value={form.prenom} onChange={e => setForm({ ...form, prenom: e.target.value })} placeholder={tText('Prénom')} />
            </div>
            <div>
            ...
            <div>
              <label className="label">{tText('Famille')}</label>
    ...
    <div className="animate-slide-up">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{tText('Âmes')}</h2>
        ...
        <div className="flex gap-2">
          ...
          {view !== 'corbeille' && (
            <button onClick={() => { setForm(emptyForm); setEditingId(null); setView('create'); }} className="btn-primary btn-sm">
              <Plus className="w-4 h-4" /> {tText('Nouvelle âme')}
            </button>
          )}
        </div>
      </div>
              ...
              <tbody>
                {(data?.content || []).map(soul => (
                  <tr key={soul.id} className="hover:bg-white/40 dark:hover:bg-gray-800/20 transition-colors">
                    ...
                    <td className="text-sm text-gray-500">{new Date(soul.dateIntegration).toLocaleDateString(getI18nLocale(), { day: 'numeric', month: 'short', year: 'numeric' })}</td>
                    <td className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button onClick={() => setShowDeleteConfirm(soul.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20" title={tText('Supprimer')}><Trash2 className="w-3.5 h-3.5 text-red-400" /></button>
                ...
                {(data?.content || []).length === 0 && (
                  <tr><td colSpan={7} className="py-12 text-center text-gray-400">{tText('Aucune âme trouvée')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
