...
export function ChecklistsTab({ deptId, onChanged }: { deptId: string; onChanged: () => void }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(tText('Checklist créée ✅'));
      setTitre(''); setItems(['', '']);
      invalidate();
    },
  ...
  const closeMutation = useMutation({
    ...
    onSuccess: () => { toast.success(tText('Checklist terminée')); invalidate(); },
    onError: (err) => toast.error(getErrorMessage(err)),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success(tText('Checklist supprimée')); invalidate(); },
    onError: (err) => toast.error(getErrorMessage(err)),
  });
    ...
    <div className="space-y-4">
      <div className="glass-card p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">{tText('Nouvelle checklist')}</h3>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
          ...
          <div>
            ...
            <select className="input" value={cibleType} onChange={(e) => setCibleType(e.target.value)}>
              <option value="GENERAL">{tText('Général')}</option>
              <option value="TACHE">{tText('Tâche')}</option>
              <option value="EVENEMENT">{tText('Événement')}</option>
        ...
        <div className="space-y-2 mb-3">
          ...
          <button onClick={() => setItems([...items, ''])} className="text-xs text-primary-600 dark:text-primary-300 flex items-center gap-1 cursor-pointer">
            <Plus className="w-3.5 h-3.5" /> {tText('Ajouter un élément')}
          </button>
        </div>
        <button onClick={() => createMutation.mutate()} disabled={!titre.trim() || createMutation.isPending}
          ...
          <Plus className="w-4 h-4" /> {tText('Créer la checklist')}
        </button>
      </div>
      ...
      {checklists.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-400">{tText('Aucune checklist pour le moment')}</p>
        </div>
      ) : (
        checklists.map((c) => (
          <div key={c.id} className="glass-card p-4">
            ...
            <div className="flex items-center gap-2 mt-3">
              <form onSubmit={(e) => {
                ...
                <input className="input py-1.5 text-xs flex-1" placeholder={tText('Ajouter un élément…')} />
              </form>
              {c.statut !== 'TERMINEE' && (
